...
def empty(msg):
    return f"<div style='text-align:center;padding:40px 0;color:#3d5a72;font-size:15px;'>{msg}</div>"
        ...
        <div style="background:#fff;border:1px solid #d8e2ee;border-radius:12px;
            ...
            <a href="{d['maps_link']}" target="_blank" style="text-decoration:none;">
                <div style="background:#eaeff5;border:1px solid #d8e2ee;color:#0d1b2a;
                            padding:9px 18px;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;">📍 Maps</div>
            </a>
        </div>"""
    return html + "</div>"
...
def hist_to_html(hist):
    ...
    for h in hist:
        ...
        html += f'<div class="hist-row"><span style="color:#3d5a72;">{h["time"]}</span><span style="font-weight:700;color:{c};text-transform:uppercase;letter-spacing:0.05em;">{h["severity"]}</span><span style="color:#3d5a72;">{h["conf"]}</span></div>'
    return html
...
def analyze_skin_ui(img, hist):
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;
        ...
        <div style="display:flex;align-items:center;gap:13px;">
            <div style="width:12px;height:12px;border-radius:50%;background:{s['color']};
    ...
    <div style="margin-bottom:24px;">
        <div style="display:flex;justify-content:space-between;margin-bottom:9px;">
            ...
            <span style="font-size:14px;font-weight:700;color:{s['color']};">{conf_val:.1f}%</span>
        </div>
        <div style="background:#eaeff5;height:8px;border-radius:5px;border:1px solid #d8e2ee;overflow:hidden;">
            <div style="width:{conf_val}%;height:100%;border-radius:5px;background:linear-gradient(90deg,#00b4a6,#0a2540);"></div>
        </div>
    </div>"""
    ...
    for cls, val in res["top3"]:
        ...
        t3 += f'<div style="display:flex;align-items:center;gap:13px;margin-bottom:12px;"><div style="font-size:15px;color:#1e3a52;font-weight:500;width:86px;">{cls.capitalize()}</div><div style="flex:1;height:7px;background:#eaeff5;border-radius:4px;border:1px solid #d8e2ee;overflow:hidden;"><div style="width:{v}%;height:100%;background:{c};border-radius:4px;"></div></div><div style="font-size:14px;color:#3d5a72;font-weight:600;width:44px;text-align:right;">{v:.1f}%</div></div>'
    t3 += "</div>"
    ...
    for m in med_data.get("medicines", []):
        ...
        <div style="background:#fff;border:1px solid #d8e2ee;border-radius:11px;
            ...
            <div style="display:inline-block;font-size:11px;font-weight:700;letter-spacing:0.08em;
                        text-transform:uppercase;background:rgba(0,180,166,0.09);color:#007a70;
                        border:1px solid rgba(0,180,166,0.22);padding:3px 10px;border-radius:4px;margin-bottom:11px;">{m['type']}</div>
            <div style="font-size:14px;color:#1e3a52;line-height:1.65;margin-bottom:10px;"><b style="color:#0d1b2a;">Use:</b> {m['usage']}</div>
    ...
    for p in med_data.get("precautions", []):
        p_html += f'<div style="display:flex;align-items:flex-start;gap:12px;padding:11px 0;border-bottom:1px solid #eaeff5;font-size:15px;color:var(--text);line-height:1.55;">
    p_html += "</div>"
...
LOGO_SVG = """<svg width="36" height="36" viewBox="0 0 36 36" fill="none">
  ...
  <path d="M7 12h9c3.3 0 6 2.7 6 6s-2.7 6-6 6H7"
        fill="none" stroke="#00d4c4" stroke-width="2.6" stroke-linejoin="round"/>
